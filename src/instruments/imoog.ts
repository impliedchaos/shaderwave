// Moog — analog polysynth. 3 detuned oscillators into a ladder filter with
// separate filter & amp ADSRs, glide and a noise mix. The ladder is per-sample
// recursive (recursive: true). Uses the extra universal banks: p2 = (osc1/2/3
// wave, glide s), p3 = (osc1/2/3 octave, noise mix); freqFrom = glide source.
import type { InstrumentDef } from '../types.js';
import shader from '../gl/shaders/synth-moog.glsl?raw';

export const imoog: InstrumentDef = {
  type: 'moog',
  name: 'Moog',
  short: 'MŌG',
  label: 'Moog — Analog Polysynth',
  blurb: '3 detuned saw oscillators into a ladder filter with separate filter & amp ADSRs. Cutoff, Reso, EnvAmt, Detune, AmpSus, FiltDecay, AmpDecay.',
  shader,
  recursive: true,
  defaults: { p0: [800, 0.45, 0.5, 0], p1: [8, 0.8, 0.6, 0.9], p2: [1, 1, 1, 0], p3: [2, 2, 2, 0] },
  paramDefs: [
    { label: 'Cutoff', bank: 'p0', i: 0, min: 30, max: 6000, step: 1 },
    { label: 'Reso', bank: 'p0', i: 1, min: 0, max: 0.98, step: 0.01 },
    { label: 'EnvAmt', bank: 'p0', i: 2, min: 0, max: 1, step: 0.01 },
    { label: 'KbdTrack', bank: 'p0', i: 3, min: 0, max: 1, step: 0.01 },
    { label: 'Detune', bank: 'p1', i: 0, min: 0, max: 30, step: 0.5 },
    { label: 'AmpSus', bank: 'p1', i: 1, min: 0, max: 1, step: 0.01 },
    { label: 'FiltDecay', bank: 'p1', i: 2, min: 0.05, max: 2, step: 0.01 },
    { label: 'AmpDecay', bank: 'p1', i: 3, min: 0.05, max: 2, step: 0.01 },
    { label: 'Osc1 Wave', bank: 'p2', i: 0, min: 0, max: 4, step: 1 },
    { label: 'Osc2 Wave', bank: 'p2', i: 1, min: 0, max: 4, step: 1 },
    { label: 'Osc3 Wave', bank: 'p2', i: 2, min: 0, max: 4, step: 1 },
    { label: 'Glide', bank: 'p2', i: 3, min: 0, max: 1.5, step: 0.01 },
    { label: 'Osc1 Oct', bank: 'p3', i: 0, min: 0, max: 4, step: 1 },
    { label: 'Osc2 Oct', bank: 'p3', i: 1, min: 0, max: 4, step: 1 },
    { label: 'Osc3 Oct', bank: 'p3', i: 2, min: 0, max: 4, step: 1 },
    { label: 'Noise', bank: 'p3', i: 3, min: 0, max: 1, step: 0.01 },
  ],
  autoTargets: [
    { code: 'CUT', label: 'Cutoff',       bank: 'p0', index: 0, min: 30, max: 6000, curve: 'log', unit: 'Hz' },
    { code: 'RES', label: 'Resonance',    bank: 'p0', index: 1, min: 0,  max: 0.95, curve: 'lin' },
    { code: 'FEN', label: 'Filter Env',   bank: 'p0', index: 2, min: 0,  max: 1,    curve: 'lin' },
    { code: 'DTC', label: 'Detune',       bank: 'p1', index: 0, min: 0,  max: 30,   curve: 'lin', unit: 'ct' },
    { code: 'SUS', label: 'Amp Sustain',  bank: 'p1', index: 1, min: 0,  max: 1,    curve: 'lin' },
    { code: 'FDC', label: 'Filter Decay', bank: 'p1', index: 2, min: 0.05, max: 2,  curve: 'lin', unit: 's' },
    { code: 'ADC', label: 'Amp Decay',    bank: 'p1', index: 3, min: 0.05, max: 2,  curve: 'lin', unit: 's' },
  ],
  presets: [
    { name: 'Classic Poly Pluck', p0: [800, 0.45, 0.5, 0], p1: [8, 0.8, 0.6, 0.9], p2: [1, 1, 1, 0], p3: [2, 2, 2, 0], fx: { distOn: true, chorusOn: true, delayOn: true, reverbOn: true, tremoloOn: true, widthOn: true, dist: 0.001, tone: 0.5, level: 1.0, width: 1.0, master: 0.32 } },
    { name: 'Industrial Laser Lead', p0: [600, 0.6, 0.7, 0], p1: [25, 0.4, 1.2, 0.8], fx: { distOn: true, chorusOn: true, delayOn: true, reverbOn: true, tremoloOn: true, widthOn: true, dist: 7.0, tone: 0.65, level: 1.0, width: 1.4, master: 0.7, chorusMix: 0.6, chorusRate: 0.8, chorusDepth: 5.0, tremoloMix: 0.3, tremoloRate: 2.5, delayTime: 0.5, delayFeedback: 0.5, delayMix: 0.3, reverbDecay: 0.95, reverbDamp: 0.2, reverbSend: 0.9, reverbMix: 0.6 } },
    { name: 'Cinematic Ambient Pad', p0: [400, 0.6, 0.5, 0], p1: [15, 0.8, 1.2, 0.9], fx: { distOn: true, chorusOn: true, delayOn: true, reverbOn: true, tremoloOn: true, widthOn: true, dist: 3.0, tone: 0.45, level: 1.0, width: 1.2, master: 0.9, chorusMix: 0.4, chorusRate: 0.5, chorusDepth: 3.0, delayTime: 0.5, delayFeedback: 0.3, delayMix: 0.15, reverbDecay: 0.95, reverbDamp: 0.2, reverbSend: 0.9, reverbMix: 0.5 } },
    { name: 'Muffled Noir Bass', p0: [400, 0.45, 0.5, 0], p1: [8, 0.8, 0.6, 0.9], fx: { distOn: true, chorusOn: true, delayOn: true, reverbOn: true, tremoloOn: true, widthOn: true, reverbMix: 0.4, reverbDecay: 0.9 } },
    { name: 'Cyberpunk Ladder Bass', p0: [800, 0.5, 0.6, 0], p1: [20, 0.8, 0.6, 0.9], fx: { distOn: true, chorusOn: true, delayOn: true, reverbOn: true, tremoloOn: true, widthOn: true, dist: 7.0, tone: 0.5, level: 1.0, chorusMix: 0.4 } },
    { name: 'Punchy Retro Synthwave', p0: [1200, 0.4, 0.5, 0], p1: [5, 0.8, 0.1, 0.2], fx: { distOn: true, chorusOn: true, delayOn: true, reverbOn: true, tremoloOn: true, widthOn: true, dist: 2.5, tone: 0.5, level: 1.0 } },
    { name: 'Deep Space Drone', p0: [300, 0.3, 0.4, 0], p1: [30, 0.9, 1.8, 1.8], fx: { distOn: true, chorusOn: true, delayOn: true, reverbOn: true, tremoloOn: true, widthOn: true, reverbDecay: 0.97, reverbMix: 0.6 } },
    { name: 'Liquid Drum & Bass Sub', p0: [150, 0.0, 0.0, 0], p1: [0, 0.9, 0.8, 0.8], fx: { distOn: true, chorusOn: true, delayOn: true, reverbOn: true, tremoloOn: true, widthOn: true, dist: 0.001, tone: 0.5, level: 1.0 } },
    { name: 'MoogProlapse', p0: [150, 0.7, 0.8, 0], p1: [4, 0.9, 0.5, 0.8], p2: [2, 1, 1, 0], p3: [2, 2, 1, 0] },
    { name: 'MoogGooner', p0: [120, 0.8, 0.9, 0], p1: [6, 0.95, 0.6, 0.9], p2: [2, 2, 1, 0], p3: [2, 2, 2, 0] },
    { name: 'GoonerGlide', p0: [900, 0.3, 0.4, 0.35], p1: [12, 0.5, 0.7, 0.4], p2: [1, 2, 1, 0.05], p3: [2, 2, 3, 0] },
    { name: 'MoogScreamer', p0: [1400, 0.2, 0.3, 0.4], p1: [16, 0.4, 0.8, 0.3], p2: [1, 3, 2, 0.04], p3: [2, 3, 2, 0] },
    { name: 'PerkyLead', p0: [1200, 0.4, 0.5, 0.35], p1: [8, 0.8, 0.6, 0.9], p2: [1, 1, 2, 0.05], p3: [2, 2, 3, 0] },
    { name: 'BreathAwayPad', p0: [400, 0.2, 0.3, 0.1], p1: [15, 0.8, 1.5, 1.2], p2: [1, 1, 0, 0], p3: [2, 2, 1, 0.05] },
    { name: 'CuckSoaring', p0: [900, 0.4, 0.6, 0.45], p1: [6, 0.9, 0.8, 0.6], p2: [1, 1, 2, 0.08], p3: [2, 3, 2, 0] },
    { name: 'SuicideWarm', p0: [180, 0.15, 0.7, 0], p1: [2, 0.95, 0.8, 1.2], p2: [2, 1, 1, 0], p3: [2, 2, 1, 0] },
    { name: 'EtherealSuicide', p0: [120, 0.08, 0.85, 0.1], p1: [1, 0.98, 1.2, 1.5], p2: [1, 1, 0, 0], p3: [2, 2, 2, 0.04] },
    { name: 'AntiseptikSoar', p0: [900, 0.35, 0.45, 0.45], p1: [15, 0.6, 0.8, 0.6], p2: [1, 1, 2, 0.07], p3: [2, 3, 2, 0] },
    { name: 'NutFunkBass', p0: [300, 0.25, 0.6, 0], p1: [4, 0.9, 0.65, 0.9], p2: [2, 1, 1, 0.02], p3: [2, 2, 1, 0] },
    { name: 'BritpopLead', p0: [1200, 0.4, 0.5, 0.4], p1: [12, 0.55, 0.75, 0.5], p2: [1, 2, 2, 0.05], p3: [2, 2, 3, 0] },
    { name: 'MurderGrowl', p0: [180, 0.15, 0.8, 0], p1: [2, 0.95, 0.8, 1.2], p2: [2, 2, 1, 0], p3: [2, 2, 1, 0.04] },
    { name: 'ZeppelinLead', p0: [950, 0.3, 0.5, 0.45], p1: [15, 0.7, 0.75, 0.5], p2: [1, 1, 2, 0.06], p3: [2, 3, 2, 0] },
    { name: 'LatchkeyBass', p0: [150, 0.05, 0.8, 0], p1: [1, 0.98, 0.8, 1], p2: [0, 0, 1, 0], p3: [2, 1, 2, 0] },
    { name: 'TailpipePulse', p0: [1000, 0.25, 0.45, 0.4], p1: [12, 0.6, 0.75, 0.5], p2: [3, 4, 3, 0.05], p3: [2, 2, 3, 0] },
    { name: 'SubGymnopedie', p0: [150, 0.2, 0.5, 0], p1: [2.0, 0.9, 0.8, 0.8], p2: [2, 1, 1, 0], p3: [2, 2, 1, 0] },
    { name: 'Axe Bass', p0: [600, 0.6, 0.7, 0.2], p1: [12.0, 0.8, 0.6, 0.8], p2: [2, 1, 2, 0.02], p3: [2, 2, 2, 0] },
  ],
};
